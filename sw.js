...
self.addEventListener('fetch',e=>{const r=e.request;if(r.method!=='GET')return;const u=new URL(r.url);
  if(u.origin===self.location.origin){ if(r.mode==='navigate'){e.respondWith(caches.match('./index.html').then(c=>c||fetch(r).catch(()=>caches.match('./index.html'))));return;} e.respondWith(caches.match(r).then(c=>c||fetch(r).then(res=>{const cp=res.clone();caches.open(CACHE_VERSION).then(ch=>ch.put(r,cp));return res;}).catch(()=>caches.match('./index.html'))));}
